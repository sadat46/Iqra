  ...
  .onCreate(async (snap, context) => {
    try {
      ...
      const message: admin.messaging.Message = {
        ...
        android: {
          notification: {
            channelId: data?.type === 'chat' ? 'chat_channel' : 'general_channel',
            sound: notificationSettings?.sound !== false ? 'default' : undefined,
            priority: 'high',
            defaultSound: true,
            defaultVibrateTimings: notificationSettings?.vibration !== false,
        ...
        apns: {
          payload: {
            aps: {
              sound: notificationSettings?.sound !== false ? 'default' : undefined,
              badge: notificationSettings?.badge !== false ? 1 : undefined,
              alert: {
                title: title,
                body: body,
              },
...
export const updateFCMToken = functions.firestore
